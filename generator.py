...
from markdown_processor import convert_markdown
from template_engine import init_template_engine, render_template
...
def build_site():
    ...
    collections_data = {}
    for coll_file in Path("collections").glob("*.json"):
    ...
    template_env = init_template_engine(config, collections_data)
    
    # 4. Process Markdown articles
    for md_file in Path("content/articles").rglob("*.md"):
        ...
        context = {**front_matter, "content": html_content}
    ...
    for md_file in Path("content/pages").rglob("*.md"):
        front_matter, md_content = parse_content(md_file)
